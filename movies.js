module.exports = [
  {
    title: "Песочник",
    year: 2016,
    originalTitle: "Piper",
    country: "США",
    genres: "мультфильм, семейный, короткометражка",
    duration: "6 мин.",
    description:
      "Маленькому птенцу предстоит преодолеть свои страхи, чтобы научиться добывать еду и увидеть красоту окружающего мира.",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/984759.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/984759",
  },
  {
    title: "Бумажный роман",
    year: 2012,
    originalTitle: "Paperman",
    country: "США, Япония",
    genres:
      "мелодрама, фэнтези, комедия, мультфильм, семейный, короткометражка",
    duration: "7 мин.",
    description:
      "Середина двадцатого столетия. Нью-Йорк. Одинокий молодой человек на платформе вокзала. Еще мгновение – и судьба подарит ему удивительный шанс, который, возможно, бывает раз в жизни. Он встретит красавицу, чей взгляд мгновенно покорит его сердце. Но девушка его мечты так же быстро исчезает, как и появляется. Будет ли у героя мультфильма вторая попытка?",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/680642.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/680642",
  },
  {
    title: "Дом из маленьких кубиков",
    year: 2008,
    originalTitle: "Tsumiki no ie",
    country: "Япония",
    genres: "драма, мультфильм, короткометражка",
    duration: "12 мин.",
    description:
      "Одинокий старик живёт в доме, который уходит корнями в глубину из-за постоянно поднимающего уровня воды, поэтому он вынужден строить этаж за этажом. Так и живёт. Однажды, во время очередной достройки своего импровизированного небоскреба, он нечаянно роняет свою любимую трубку.",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/439755.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/439755",
  },
  {
    title: "Облачно с прояснениями",
    year: 2009,
    originalTitle: "Partly Cloudy",
    country: "США",
    genres: "фэнтези, комедия, мультфильм, семейный, короткометражка",
    duration: "6 мин.",
    description:
      "Аисты приносят детей людям, животным, рыбам, а замечательных деток интересными способами делают облака. Все делают милых и пушистых детей, но есть одно облако, которое отличается от остальных облаков.",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/462582.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/462582",
  },
  {
    title: "Меню",
    year: 2014,
    originalTitle: "Feast",
    country: "США, Япония",
    genres: "драма, мелодрама, комедия, мультфильм, семейный, короткометражка",
    duration: "6 мин.",
    description:
      "История любви из жизни одного человека глазами его лучшего друга, собаки по имени Винстон, показанная через еду, которую они делят.",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/842712.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/842712",
  },
  {
    title: "Старик и море",
    year: 1999,
    originalTitle: "The Old Man and the Sea",
    country: "Канада, Япония, Россия",
    genres: "драма, приключения, мультфильм, короткометражка",
    duration: "20 мин.",
    description:
      "История дружбы между маленьким деревенским мальчишкой и старым рыбаком. Сантьяго, сильный и гордый человек, он не может смириться с неумолимым течением времени, отнимающем физические силы. Уже много недель он возвращается с моря без улова. Наконец он принимает решение - уйти за добычей далеко-далеко в море и без улова не возвращаться. Только так старый рыбак может вернуть себе уверенность и самоуважение. Рано утром он прощается со своим маленьким другом, с родным берегом, который видит, возможно, в последний раз, и растворяется в темноте морских вод. И все же удача приходит к рыбаку. В его снасти попадается гигантская рыба. Двое суток в море продолжается их поединок, но рыба не сдается и тянет Сантьяго все дальше и дальше в море. Но старый рыбак убедил себя в том, что упорство и сила духа - это то, что принесет ему победу…",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/26270.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/26270",
  },
  {
    title: "Киви!",
    year: 2006,
    originalTitle: "Kiwi!",
    country: "США",
    genres: "драма, приключения, мультфильм, короткометражка",
    duration: "3 мин.",
    description: "Мультик о птичке, которая мечтала летать.",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/466400.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/466400",
  },
  {
    title: "О птичках",
    year: 2000,
    originalTitle: "For the Birds",
    country: "США",
    genres: "комедия, мультфильм, семейный, короткометражка",
    duration: "3 мин.",
    description:
      "Позитивный красочный мульт, про птичек на проводе. Мелкие воробьи vs Большая птица. Но хорошо смеется тот, кто смеется последний.",
    posterUrl: "https://kinopoiskapiunofficial.tech/images/posters/kp/672.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/672",
  },
  {
    title: "Отец и дочь",
    year: 2000,
    originalTitle: "Father and Daughter",
    country: "Великобритания, Нидерланды, Бельгия",
    genres: "драма, мультфильм, короткометражка",
    duration: "8 мин.",
    description:
      "Маленькая девочка тоскует по исчезнувшему отцу. Сменяются времена года, проходят годы… Девочка становится взрослой, обзаводится семьей, стареет, но глубокая печаль по отцу остается с ней.",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/120540.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/120540",
  },
  {
    title: "Игра Джери",
    year: 1997,
    originalTitle: "Geri's Game",
    country: "США",
    genres: "комедия, мультфильм, семейный, короткометражка",
    duration: "5 мин.",
    description:
      "Действие фильма начинается в пустом осеннем парке. Джери, одинокий старичок, который из-за отсутствия соперников начинает играть в шахматы сам с собой. Создаётся впечатление, что в шахматы играют два совершенно разных человека. Джери, играющий белыми, отличается от своего «оппонента», играющего чёрными. Джери, играющий белыми, кроткий и нерешительный, носящий очки, а Джери, играющий чёрными, — нахальный и самоуверенный холерик.",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/78163.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/78163",
  },
  {
    title: "Джонни экспресс",
    year: 2014,
    originalTitle: "Jyani ikseupeureseu",
    country: "Корея Южная",
    genres: "фантастика, комедия, мультфильм, короткометражка",
    duration: "5 мин.",
    description:
      "Курьер Джонни путешествует по разным планетам и галактикам, чтобы доставлять межгалактическую почту. Он ленив и его единственное желание — спать в автопилотируемом космическом корабле. Когда корабль прибывает в пункт назначения, всё, что он должен сделать — это просто вручить посылку. Тем не менее, Джонни попадаются причудливые планеты, где всё идёт не по плану.",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/840027.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/840027",
  },
  {
    title: "Бао",
    year: 2018,
    originalTitle: "Bao",
    country: "США",
    genres: "драма, фэнтези, мультфильм, семейный, короткометражка",
    duration: "8 мин.",
    description:
      "Женщина получает шанс стать матерью, когда один из её пельмешков оживает.",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/1115649.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/1115649",
  },
  {
    title: "Голова-гора",
    year: 2002,
    originalTitle: "Atama-yama",
    country: "Япония",
    genres: "фэнтези, комедия, мультфильм, короткометражка, аниме",
    duration: "10 мин.",
    description:
      "Очень скупой человек проглатывает несколько вишневых косточек вместо того, чтобы выбросить их, и одна из них пускает побеги у него на голове. Дерево растет, а в период цветения люди приходят отдохнуть в его тени…",
    posterUrl:
      "https://kinopoiskapiunofficial.tech/images/posters/kp/149705.jpg",
    watchUrl: "https://reyohoho.github.io/reyohoho/movie/149705",
  },
];
